...
{
	if(checkdebug(dl,debugnum_pfda_bin_single_e)){pfda_debug_msg("Entering pfda_bin_single_e\n");fflush(stdout);}
	...
	for(int obs=0;obs<*N;obs++){
		if(checkdebug(dl,debugnum_pfda_bin_single_e)){pfda_debug_msg("estimating hat{alpha alpha^T} for obs = %d\n",obs);fflush(stdout);}
		...
		if(checkdebug(dl,debugnum_pfda_bin_single_e)){pfda_debug_msg("S1[[%d]]\n",obs);printmat(S1+obs**k,*k,one);fflush(stdout);}
	...
	if(checkdebug(dl,debugnum_pfda_bin_single_e)){pfda_debug_msg("leaving pfda_bin_single_e\n");fflush(stdout);}
}
	...
	int const * const dl, double * dp, int * ip)
{
	...
	if(checkdebug(dl,debugnum_pfda_bin_m5)){ pfda_debug_msg("leaving pfda_bin_m5\n"); fflush(stdout);}
}
...
if(*dl){
	...
	if(checkdebug(dl,debugnum_bin_single_inputs_large)){
...
{ // find initial values
	if(checkdebug(dl,debugnum_bin_single_steps)){pfda_debug_msg("Estimating initial values\n");fflush(stdout);}
...
while(I < *MaxIter)/*  */{
	...
	{/* W Step - W|y / WW|Y */
		if(checkdebug(dl,debugnum_pfda_bin_single_break_before_ew)){
	...
	{/* step 1/E - alpha / Sigma_aa / aa_hats*/
		...
		if(checkdebug(dl,debugnum_bin_single_steps)){pfda_debug_msg("Step E\n");fflush(stdout);}
		pfda_bin_single_e( aa_hats, alpha, Sigma_aa, w, ww, nobs, M, N, k, B, p, tm, tf, Da, dl, dp, ip);
		if(checkdebug(dl, debugnum_bin_single_estimates)){
			pfda_debug_msg("alpha: \n");printmat(alpha,*p,*k);
			pfda_debug_msg("Sigma_aa: \n");printmat(Sigma_aa,*N,*k**k);
	...
	{/* step 2 - tm*/
		if(checkdebug(dl,debugnum_pfda_bin_single_break_before_2)){pfda_debug_msg("debuging information has indicated to exit prematurly before step 2.\n");fflush(stdout);return;}
		if(checkdebug(dl,debugnum_bin_single_steps)){pfda_debug_msg("Step 2\n");fflush(stdout);}
		pfda_m2(tm,w,nobs,M,N,k,B,p,lm,ISD,tf,seps,alpha,dl, dp);
		if(checkdebug(dl, debugnum_bin_single_estimates)){pfda_debug_msg("tm: \n");printmat(tm,*p,one);fflush(stdout);}
	}
	{/* step 3 -tf */
		if(checkdebug(dl,debugnum_pfda_bin_single_break_before_3)){pfda_debug_msg("debuging information has indicated to exit prematurly before step 3.\n");fflush(stdout);return;}
		if(checkdebug(dl,debugnum_bin_single_steps)){pfda_debug_msg("Step 3\n");fflush(stdout);}
		pfda_m3_core(tf, aa_hats, w, nobs, M, N, k, B, p, lf, ISD, tm, seps, alpha, btb, dl, dp, ip);
		if(checkdebug(dl, debugnum_bin_single_estimates)){pfda_debug_msg("tf: \n");printmat(tf,*p,*k);fflush(stdout);}
	}
	{/* step 4 - Da/ orthogonalize tf*/
		if(checkdebug(dl,debugnum_pfda_bin_single_break_before_4)){pfda_debug_msg("debuging information has indicated to exit prematurly before step 4.\n");fflush(stdout);return;}
		if(checkdebug(dl,debugnum_bin_single_steps)){pfda_debug_msg("Step 4&5\n");fflush(stdout);}
	...
	{/* Compute Convergence Criteria */
		if(checkdebug(dl,debugnum_bin_single_steps)){pfda_debug_msg("Computing convergence Criteria\n");fflush(stdout);}
		...
		/* Debug */
		if(checkdebug(dl,debugnum_bin_single_convergence_components)){
		...
		if(checkdebug(dl,debugnum_bin_single_convergence)){pfda_debug_msg("Criteria:%g\n", convergenceCriteria);fflush(stdout);}
	}
	I++;
